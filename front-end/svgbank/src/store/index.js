...
export async function getIcons() {
    const response = await fetch("http://192.168.4.169:4000/api/icons/");
    if (!response.ok) {
...
export async function getCategory() {
    const response = await fetch("http://192.168.4.169:4000/api/icons/category/");
    if (!response.ok) {
        throw new Error('Could not fetch data');
    }
    const data = await response.json();
...
export async function updateIcon(iconId, icon) {
    const response = await fetch(`http://192.168.4.169:4000/api/icons/${iconId}/`, {
        method: "PUT",
        headers: {
            "Content-Type": "application/json"
        },

  ...
  <teleport to="body">
    <div v-if="show" class="fixed inset-0 bg-[rgba(24,49,83,0.5)] flex items-center justify-center z-50">
      <div class="bg-white rounded-2xl max-w-4xl w-full h-96 flex relative overflow-hidden">
        ...
        <div
          class="flex-1 flex items-center justify-center m-8 border-2 border-[rgba(195,198,209,0.5)] rounded-2xl">
          <div class="relative w-48 h-48" v-html="icon.svg"></div>
          <FontAwesomeIcon :icon="faPalette" size="xl" style="color: #74c0fc !important;" class="absolute bottom-12 left-12 !text-[#74c0fc] !important" :style="{ color: '#74c0fc', fill: '#74c0fc' }"/>
        </div>
        ...
        <div class="flex-1 flex flex-col p-8">
          ...
          <button @click="navigator.clipboard.writeText(icon.svg)"

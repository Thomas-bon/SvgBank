...
async function copyToClipboard() {
  try {
    await navigator.clipboard.writeText(coloredSvg.value)
    copied.value = true
...
<template>
  <teleport to="body">
    <div v-if="show" class="fixed inset-0 bg-[rgba(24,49,83,0.5)] flex items-center justify-center z-50">
      <div class="bg-white rounded-2xl max-w-4xl w-full h-96 flex relative overflow-hidden">
        <button @click="closeModal" class="absolute top-2 right-3 text-gray-500 text-2xl">
          &times;
        </button>

        <div
          class="relative flex-1 flex items-center justify-center m-8 border-2 border-[rgba(195,198,209,0.5)] rounded-2xl">
          <div class="relative w-48 h-48" v-html="coloredSvg"></div>

          <FontAwesomeIcon :icon="faPalette" size="xl"
            class="absolute bottom-5 left-5 cursor-pointer !text-[rgb(20,110,190)]" @click="openColorPicker" />
          <input ref="inputRef" type="color" v-model="color" class="absolute opacity-0 w-0 h-0" />
        </div>
